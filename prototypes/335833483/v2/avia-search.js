...
function airportChoices(query) {
  ...
  return Object.entries(airportCatalog).flatMap(([city, airports]) => [airports.primary, airports.secondary].filter((item, index, array) => array.findIndex(candidate => candidate.code === item.code) === index).map(item => ({ city, ...item }))).filter(item => !normalized || `${item.city} ${item.name} ${item.code}`.toLocaleLowerCase('ru').includes(normalized))
}
function closeAirportPopover() { if (!airportPopover) return; airportPopover.hidden = true; airportField = null }
function openAirportPopover(field, showAll = false) { if (!airportPopover) return; airportField = field; const input = searchForm.elements[field]; const bounds = input.getBoundingClientRect(); const choices = airportChoices(showAll ? '' : input.value); airportPopover.style.left = `${Math.round(bounds.left)}px`; airportPopover.style.top = `${Math.round(bounds.bottom + 8)}px`; airportPopover.style.width = `${Math.max(280, Math.round(bounds.width))}px`; airportPopover.innerHTML = choices.length ? choices.map(item => `<button class="avia-airport-option" type="button" role="option" data-airport-choice="${escapeHtml(field)}" data-city="${escapeHtml(item.city)}"><span><strong>${escapeHtml(item.city)}</strong><small>${escapeHtml(item.name)}</small></span><b>${escapeHtml(item.code)}</b></button>`).join('') : '<span class="avia-airport-option"><span><strong>Аэропорт не найден</strong><small>Измените запрос</small></span></span>'; airportPopover.hidden = false }

function cityCase(city, direction) {
  return cityCases[city]?.[direction] || city
}

function travellerCount(value) {
  const counts = String(value || '').match(/\d+/gu)?.map(Number) || []
  return Math.min(9, Math.max(1, counts.reduce((sum, count) => sum + count, 0) || 1))
}

function conditionShift(value, range) {
  let hash = 2166136261
...
function buildFlights() {
  ...
  const flights = [
    ...
    },
    {
      ...
      handLuggage: { positive: true, dimensions: '55×40×20 см' }, outbound: { airline: 'Россия', logo: 'rossiya', number: 'FV 6105', depart: '08:20', duration: '4 ч 10 м', arrival: '12:30', from: originPrimary, to: destinationSecondary }, returning: isRoundTrip ? { airline: 'Россия', logo: 'rossiya', number: 'FV 6106', depart: '19:10', duration: '4 ч 05 м', arrival: '23:15', from: destinationSecondary, to: originPrimary } : null,
    },
    {
      ...
      handLuggage: { positive: true, dimensions: '55×40×23 см' }, outbound: { airline: 'S7', logo: 's7', number: 'S7 1011', depart: '11:35', duration: '3 ч 45 м', arrival: '15:20', from: originSecondary, to: destinationPrimary }, returning: isRoundTrip ? { airline: 'S7', logo: 's7', number: 'S7 1012', depart: '17:10', duration: '3 ч 50 м', arrival: '21:00', from: destinationPrimary, to: originSecondary } : null,
    },
    {
      ...
      handLuggage: { positive: true, dimensions: '36×30×27 см' }, outbound: { airline: 'Победа', logo: 'pobeda', number: 'DP 981', depart: '18:25', duration: '4 ч 30 м', arrival: '22:55', from: originPrimary, to: destinationPrimary }, returning: isRoundTrip ? { airline: 'Победа', logo: 'pobeda', number: 'DP 982', depart: '07:15', duration: '4 ч 20 м', arrival: '11:35', from: destinationPrimary, to: originPrimary } : null,
    },
  ]
  const priceShift = conditionShift(`${state.origin}|${state.destination}|${state.depart}|${state.returning}`, 3200)
  const passengers = travellerCount(state.traveller)
  return flights.map((flight, index) => ({ ...flight, price: (flight.price + priceShift + index * 230) * passengers }))
}
...
function renderFlight(flight) {
  ...
  const baggageLabel = flight.hasBaggage ? 'Багаж включён' : 'Без багажа'
  return `
      ...
      <div class="flight-card__content">
        ...
        <aside class="flight-fare">
          <div class="flight-fare__inner">
            ...
            <div class="flight-benefit"><img src="./assets/avia/no-baggage.svg" alt=""><span>${baggageLabel}</span></div>
...
let flightRefreshAnimationTimer = 0
function refreshFlights() {
  window.clearTimeout(flightRefreshTimer)
  window.clearTimeout(flightRefreshAnimationTimer)
  flightList.classList.remove('is-refreshed')
  ...
  flightRefreshTimer = window.setTimeout(() => {
    renderFlights()
    void flightList.offsetWidth
    flightList.classList.add('is-refreshed')
    flightRefreshAnimationTimer = window.setTimeout(() => flightList.classList.remove('is-refreshed'), 520)
  }, 420)
}

function updateQuickChipPrices() {
  const flights = buildFlights()
  const directPrice = Math.min(...flights.filter(flight => flight.stops === 0).map(flight => flight.price))
  const baggagePrice = Math.min(...flights.filter(flight => flight.hasBaggage).map(flight => flight.price))
  document.querySelector('[data-filter-chip="direct"] span').textContent = Number.isFinite(directPrice) ? `от ${formatPrice(directPrice)}` : 'нет вариантов'
  document.querySelector('[data-filter-chip="baggage"] span').textContent = Number.isFinite(baggagePrice) ? `от ${formatPrice(baggagePrice)}` : 'нет вариантов'
}
...
;['origin', 'destination'].forEach(field => { const input = searchForm.elements[field]; input.addEventListener('focus', () => openAirportPopover(field, true)); input.addEventListener('input', () => openAirportPopover(field)) })
...
filtersForm.addEventListener('input', () => {
  const selectedStops = filtersForm.elements.stops.value
  state.directChip = selectedStops === 'direct'
  document.querySelector('[data-filter-chip="direct"]').setAttribute('aria-pressed', String(state.directChip))
  updateRangeCopies()
  refreshFlights()
})
...
document.addEventListener('click', event => {
  ...
  if (airportChoice) { const field = airportChoice.dataset.airportChoice; state[field] = airportChoice.dataset.city; setSearchValues(); syncQuery(); refreshFlights(); closeAirportPopover(); announce(`Выбран аэропорт: ${airportChoice.textContent.trim()}`); return }
  const airportInput = event.target.closest('#avia-origin, #avia-destination')
  if (airportInput) { openAirportPopover(airportInput.name, true); return }
  ...
  if (chip) {
    ...
    if (chip.dataset.filterChip === 'direct') {
      ...
      filtersForm.elements.stops.value = active ? 'direct' : 'one'
