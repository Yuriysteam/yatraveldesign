...
function travellerCount(value) {
  ...
  return Math.min(12, Math.max(1, counts.reduce((sum, count) => sum + count, 0) || 1))
}
...
function pricedHotels() {
  const nights = calculateNights()
  const rooms = Math.max(1, Math.ceil(travellerCount(state.traveller) / 2))
  const signature = `${state.city}|${state.checkin}|${state.checkout}`
  const baseShift = conditionShift(signature, 1800)
  return hotels.map((hotel, index) => {
    const nightly = hotel.nightly + baseShift + index * 120
    const price = nightly * nights * rooms
    const oldPrice = hotel.oldPrice
      ? Math.round((price * hotel.oldPrice / hotel.price) / 100) * 100
      : null
    return {
      ...hotel,
      nightly,
...
function filterHotels() {
  ...
  const filtered = pricedHotels().filter(hotel => {
...
function renderPins() {
  ...
  const hotelPins = pricedHotels().map(hotel => {
...
function selectHotel(hotelId) {
  const hotel = pricedHotels().find(item => item.id === hotelId)
  if (!hotel) return
...
document.addEventListener('click', event => {
  ...
  if (action === 'next') {
    const selected = pricedHotels().find(hotel => hotel.id === state.selectedHotelId)
    if (!selected) {
      announce('Сначала выберите отель')
      return
    }
    const target = buildTripLink(selected)
