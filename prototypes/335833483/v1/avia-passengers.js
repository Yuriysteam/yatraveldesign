...
const legacyStorageKey = 'business-trip-avia-passenger-draft-v1'
const confirmationStorageKey = 'business-trip-avia-confirmation-v1'
...
function passengerFromBlock(block) {
  ...
  return {
    ...
    middleName: noMiddleName ? '' : value('middleName'),
    noMiddleName,
...
function renderBookingContext() {
  ...
  businessTripRow.hidden = !state.workTrip
  addToTripToggle.setAttribute('aria-checked', String(state.addToTrip))
  businessTripKind.hidden = !state.addToTrip
...
document.addEventListener('click', event => {
  ...
  else if (action === 'back-to-trip') window.location.href = buildLink('./trip.html').href
