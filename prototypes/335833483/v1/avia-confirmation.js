...
const bookingStorageKey = 'business-trip-avia-booking-v1'
const personalTripsStorageKey = 'personal-trip-bookings-v1'
    ...
    if (requestedSegment === 'outbound' || requestedSegment === 'return') {
      ...
      if (isRoundTripSelection) {
        ...
        if (requestedSegment === 'return') {
          target.searchParams.set('returnFrom', state.route.origin)
          target.searchParams.set('returnTo', state.route.destination)
        } else {
          target.searchParams.set('flightFrom', state.route.origin)
          target.searchParams.set('flightTo', state.route.destination)
        }
      ...
      if (state.flightScope === 'roundtrip') {
        ...
        target.searchParams.delete('returnFrom')
