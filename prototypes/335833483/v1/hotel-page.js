...
const offersSection = document.querySelector('#hotel-offers')
const nearbyDates = document.querySelector('.nearby-dates')
...
if (!page || !offerForm || !offersSection || !nearbyDates || !dateWagonsRoot || !roomsRoot || !nearbyHotelsRoot || !guestFavoritesRoot || !announcer || !lightbox || !lightboxImage || !lightboxCaption) {
...
const startedFromExistingTrip = params.get('addToTrip') === '1'
  && params.get('tripKind') === 'existing'
  && Boolean(sourceTripId)
const isWorkTripContext = startedFromExistingTrip || params.get('workTrip') === '1' || params.get('addToTrip') === '1'
...
function renderHotelContext() {
  page.dataset.hotelContext = isWorkTripContext ? 'work-trip' : 'standalone'
  offerForm.hidden = isWorkTripContext
  nearbyDates.hidden = isWorkTripContext
  if (isWorkTripContext) {
    offersSection.removeAttribute('aria-labelledby')
    offersSection.setAttribute('aria-label', 'Номера и тарифы')
  } else {
    offersSection.setAttribute('aria-labelledby', 'offers-title')
    offersSection.removeAttribute('aria-label')
  }
}
    ...
    <div class="tariff-row${tariff.discount ? ' has-discount' : ''}${selected ? ' is-selected' : ''}" data-tariff-id="${escapeHtml(tariff.id)}">
      ...
      <div class="tariff-booking">
        <div class="tariff-price-panel">
          ...
          ${oldPrice}
          <span class="tariff-nights">3 ночи</span>
