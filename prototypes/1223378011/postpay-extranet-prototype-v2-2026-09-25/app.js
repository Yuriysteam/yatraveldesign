...
function paymentMethods() {
  ...
  const rows = [
    ["online", "Вся оплата на Яндекс Путешествиях", "Гость оплачивает бронирование в приложении или на сайте", ""],
    ["onsite", "По предоплате", "Гость вносит предоплату в приложении или на сайте, а остальное оплачивает вам при заселении", ""],
    ["both", "Вся оплата сразу или по предоплате", "Гость сам выбирает, оплатить всю сумму сразу или внести предоплату, а остальное оплатить вам при заселении", "Больше бронирований"],
  ];
  return `<section class="settings-card settings-card--methods">
    <div class="settings-card__title-row">
      ...
      <button type="button" class="payment-methods__more" data-action="payment-details">
        <span>Подробнее</span>
        <span class="payment-methods__more-icon" aria-hidden="true"></span>
      </button>
    </div>
    <div class="radio-list">
      ${rows.map(([value, title, description, badge]) => `<div class="radio-option-group">
...
function onlineSettings() {
  ...
  return `<section class="settings-card cancellation-card">
    ...
    <div class="penalty-fields"><div class="field">
      <label class="field__label">${rule.freeCancellation ? "Штраф при отмене после бесплатного периода" : "Штраф при отмене"}</label>
...
document.addEventListener("click", (event) => {
  ...
  if (action === "payment-details") { event.preventDefault(); event.stopPropagation(); return; }
