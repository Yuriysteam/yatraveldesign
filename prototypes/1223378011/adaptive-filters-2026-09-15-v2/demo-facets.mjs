...
/** Empty bounds are unbounded; values are square metres, not a price-like cap. */
export function normalizeAreaRange(range = {}) {
  if (range == null) return { min: null, max: null };
  if (typeof range !== 'object' || Array.isArray(range)) throw new TypeError('Area range must be an object.');
  const bound = value => {
    if (value == null || (typeof value === 'string' && value.trim() === '')) return null;
    if (typeof value !== 'number' && typeof value !== 'string') throw new RangeError('Area bounds must be nonnegative finite numbers or empty.');
    const number = Number(value);
    if (!Number.isFinite(number) || number < 0) throw new RangeError('Area bounds must be nonnegative finite numbers or empty.');
    return number;
  };
...
const FACETS = Object.freeze({
  ...
  sleepingPlaces: ['2', '3 или 4', '5 или 6', '7 и больше'],
...
function makeOffer(index, random) {
  ...
  // Separate streams make new filters deterministic without changing earlier
  // synthetic predicates or counters. These stay catalog filters, not recommendations.
  const requestedAmenities = randomGenerator(0xA9A90000 + index);
  add('Утюг', requestedAmenities() < (apartment || house ? 0.76 : hotel ? 0.61 : 0.24));
  add('Джакузи', requestedAmenities() < (premium ? 0.38 : rural ? 0.18 : sea && hotel ? 0.2 : 0.025));
  // A separate stream keeps every existing offer feature and price unchanged.
  // Apartment controls filter this synthetic data, not live inventory.
  const apartmentControls = randomGenerator(0xA9A80000 + index);
  const sleepingPlaces = Math.max(doubleBed ? 2 : 1, rooms * 2 + pickWeighted(apartmentControls, [-1, 0, 1, 2], [16, 50, 25, 9]));
  const area = Math.round((house ? 26 : apartment ? 18 : 10) + rooms * (house ? 18 : apartment ? 13 : 9) + apartmentControls() * 22);
  if (sleepingPlaces >= 2) {
    features.add(sleepingPlaces === 2 ? '2' : sleepingPlaces <= 4 ? '3 или 4' : sleepingPlaces <= 6 ? '5 или 6' : '7 и больше');
  }
  return Object.freeze({id: index, price, area, sleepingPlaces, filterIds: Object.freeze([...features])});
}
 ...
 * evaluate accepts an Array/Set of those ids, an inclusive total-price range,
 * and an optional inclusive area range with null/empty bounds left unbounded.
...
export function createDemoFacets(definitions) {
  ...
  function evaluate(selectedIds = [], price = {min: 0, max: MAX_PRICE}, area = {}) {
    ...
    const areaRange = normalizeAreaRange(area);
    const rangeBits = min === 0 && max >= MAX_PRICE && areaRange.min == null && areaRange.max == null ? allBits : new Uint32Array(wordCount);
    if (rangeBits !== allBits) {
      for (const record of records) {
        if (record.price >= min && record.price <= max
          && (areaRange.min == null || record.area >= areaRange.min)
          && (areaRange.max == null || record.area <= areaRange.max)) rangeBits[record.id >>> 5] |= 1 << (record.id & 31);
      }
    }
