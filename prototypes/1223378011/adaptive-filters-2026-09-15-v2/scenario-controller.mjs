import {
  ...
  MAX_CHILD_AGE,
  MIN_CHILD_AGE,
} from './settings-panel/scenario-state.mjs';
...
function normalizeSnapshot(value) {
  ...
  if (value.childAges !== undefined && !Array.isArray(value.childAges)) return null;
  const suppliedChildAges = value.childAges ?? [];
  if (suppliedChildAges.some(age => age !== null
    && (!Number.isInteger(age) || age < MIN_CHILD_AGE || age > MAX_CHILD_AGE))) return null;
  const childAges = Array.from({ length: children }, (_, index) => suppliedChildAges[index] ?? null);
