...
function normalizeChildAges(value, children) {
  const ages = Array.isArray(value) ? value : [];
  return Array.from({ length: children }, (_, index) => {
    const age = ages[index];
    if (age === null || age === undefined) return null;
    return Number.isInteger(age) && age >= MIN_CHILD_AGE && age <= MAX_CHILD_AGE ? age : null;
  });
}
...
export function cloneScenario(value = DEFAULT_SCENARIO) {
  const children = integerInRange(value.children, DEFAULT_SCENARIO.children, 0, 6);
  return {
    version: SCENARIO_VERSION,
    adults: value.adults,
    children,
    childAges: normalizeChildAges(value.childAges, children),
...
export function normalizeScenario(candidate, fallbackCandidate = DEFAULT_SCENARIO) {
  ...
  const housing = HOUSING_OPTIONS.some(option => option.id === value.housing)
    ? value.housing
    : fallback.housing;
  const children = integerInRange(value.children, fallback.children, 0, 6);
