...
function readChildAges(value, children) {
  if (value === undefined) return Array.from({ length: children }, () => null);
  if (!Array.isArray(value)) throw new TypeError('Child ages must be an array when provided.');
  if (value.length !== children) throw new RangeError('Child ages must match the number of children.');
  const childAges = [...value];
  if (childAges.some(age => age !== null && (!Number.isInteger(age) || age < 0 || age > 17))) {
    throw new RangeError('Child ages must be null or integers from 0 to 17.');
  }
  return childAges;
}

function childAgeSegment(age) {
...
function readContext(context) {
  ...
  const childAges = readChildAges(context.childAges, children);
  const childAgeSegments = childAges.map(childAgeSegment);
  ...
  return {
    adults, children, childAges, childAgeSegments, housing, geo, nights,
    family: children > 0,
    hasInfant: childAgeSegments.includes('infant'),
    hasChild: childAgeSegments.includes('child'),
    hasTeen: childAgeSegments.includes('teen'),
    hasPoolAgeChild: childAgeSegments.some(segment => segment === 'child' || segment === 'teen'),
    company: adults >= 3,
    largeCompany: adults + children >= 5,
    pair: adults === 2 && children === 0,
    // A seven-night stay already has the everyday needs of a week-long trip.
    long: nights >= 7,
    cityWeek: geo === 'city' && nights >= 7,
...
const SCORING_RULES = Object.freeze([
  ...
  scoringRule('Кухня', [
    ...
    factor('long', '7–30 ночей', 60, trip => trip.long),
  ...
  scoringRule('Стиральная машина', [
    ...
    factor('long', '7–30 ночей', 70, trip => trip.long),
  ...
  scoringRule('Ванна', [
    ...
    factor('city-week', 'Город · 7–30 ночей', 30, trip => trip.cityWeek),
    factor('short', '1–3 ночи', 15, trip => trip.short, 'boost'),
  ], 'Городская поездка от недели сама достигает порога; в короткой поездке прохладный сезон усиливает ванну.',
  trip => trip.cityWeek
    ? `В городской поездке на ${trip.nights} ночей ванна может помочь восстановиться после дня в городе.`
  ...
  scoringRule('Завтрак', [
    ...
    factor('city-week', 'Город · 7–30 ночей', 35, trip => trip.cityWeek),
    factor('city-short', 'Город · 1–3 ночи', 25, trip => trip.geo === 'city' && trip.short, 'boost'),
  ], 'Завтрак должен входить в доступный тариф; городской короткий и недельный сценарии учитываются отдельно.',
  trip => trip.cityWeek
    ? `В городской поездке на ${trip.nights} ночей включённый завтрак может сделать утро проще.`
  ...
  scoringRule('Посудомоечная машина', [
    ...
    factor('long', '7–30 ночей', 15, trip => trip.long, 'boost'),
  ], 'Порог достигается только при совпадении компании и поездки от недели.',
  ...
  scoringRule('Бассейн', [
    ...
    factor('child-2-17', 'Есть ребёнок 2–17 лет у моря или леса', 10, trip => trip.hasPoolAgeChild && ['sea', 'forest'].includes(trip.geo), 'boost'),
  ], 'Возрастное усиление действует для детей 2–17 лет; доступ и сезон работы нужно уточнить.',
  trip => trip.hasPoolAgeChild && ['sea', 'forest'].includes(trip.geo)
    ? 'Летом у моря или леса бассейн может дополнить отдых с ребёнком 2–17 лет.'
  ...
  scoringRule('Крытый', [
    ...
    factor('child-2-17', 'Есть ребёнок 2–17 лет у моря или леса', 10, trip => trip.hasPoolAgeChild && ['sea', 'forest'].includes(trip.geo), 'boost'),
  ], 'Возрастное усиление действует для детей 2–17 лет; крыша не гарантирует подогрев или работу на даты.',
  trip => trip.hasPoolAgeChild && ['sea', 'forest'].includes(trip.geo)
    ? 'В прохладный сезон крытый бассейн может быть удобнее для отдыха с ребёнком 2–17 лет; подогрев и работу на даты нужно уточнить.'
  ...
  scoringRule('Детская площадка', [
    factor('child-2-11', 'Есть ребёнок 2–11 лет', 50, trip => trip.hasChild),
    factor('pool-geo', 'У моря или леса', 15, trip => ['sea', 'forest'].includes(trip.geo), 'boost'),
  ], 'Основной возрастной сигнал — ребёнок 2–11 лет; оснащение площадки нужно уточнить.',
  'Для ребёнка 2–11 лет площадка на территории может дать ещё один вариант досуга.'),
  ...
  scoringRule('Рядом море', [
    factor('sea', 'У моря', 50, trip => trip.geo === 'sea'),
    factor('long', '7–30 ночей', 10, trip => trip.long, 'boost'),
  ...
  scoringRule('Детская кроватка', [
    factor('infant', 'Есть ребёнок 0–1 года', 85, trip => trip.hasInfant),
  ], 'Только если указан хотя бы один ребёнок 0–1 года; доступность кроватки нужно уточнить.',
  'Для ребёнка 0–1 года детская кроватка может упростить сон; возможность её предоставления нужно уточнить.'),
...
export function inspectRecommendations(context, { selected = [], counts = null } = {}) {
  ...
  // We never infer pets, cars or skiing.
